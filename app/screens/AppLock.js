...
export default function AppLock({ navigation }) {
  ...
  const promptAuth = useCallback(async () => {
    try {
      ...
      } else if (result.error && !['user_cancel', 'system_cancel'].includes(result.error)) {
    ...
    <GlassCard accessibilityLabel="App lock status">
      ...
      {available && enrolled ? (
        ...
        <HLButton
          ...
          onPress={continueWithoutBiometrics}
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        <View style={{ flex: 1, justifyContent: 'center', padding: spacing(2) }}>
          {content}
          <Text style={{ color: colors.subtext, marginTop: spacing(2), textAlign: 'center' }}>
